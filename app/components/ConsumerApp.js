...
import RobinhoodConnectionCard from './RobinhoodConnectionCard';
...
export default function ConsumerApp({ token, user, onLock }) {
  ...
  return <main className="ceApp"><Header tab={tab} setTab={setTab} user={user} onLock={onLock}/>{error && <div className="ceError"><b>Connection notice</b><span>{error}</span><button onClick={() => setError('')}>×</button></div>}<div className="cePage">
    ...
    {tab === 'invest' && <><section className="ceSectionHead"><div><p className="ceEyebrow">INVEST</p><h1>Your brokers, manual paper orders and independent bots.</h1><p>Alpaca remains the guarded paper lab. Robinhood Agentic connections are linked separately to each signed-in user.</p></div><button className="ceSecondary" onClick={() => { localStorage.removeItem(PROFILE_KEY); setProfile(null); }}>Update goals</button></section>
      <RobinhoodConnectionCard api={api}/>

  ...
  return <main className="ceApp">
    ...
    <div className="cePage">
      ...
      {tab === 'activity' && <>
        ...
        <section className="ceCard"><div className="ceCardTitle"><div><p className="ceEyebrow">SYSTEM LOG</p><h2>Why CausalEdge acted—or didn’t</h2></div><span className="cePaperPill">PAPER ENGINE</span></div>{engine?.logs?.length ? <div className="ceActivityList">{engine.logs.slice(0,30).map((log, index) => <div key={`${log.time}-${index}`}><span className={`ceActivityType ${String(log.type || '').toLowerCase()}`}>{log.type}</span><div><b>{log.symbol || 'SYSTEM'}</b><p>{log.message}</p></div><time>{shortTime(log.time)}</time></div>)}</div> : <div className="ceEmpty"><b>No engine actions yet.</b><span>That is expected while automatic and paper execution are disabled.</span></div>}</section>
      </>}
    </div>

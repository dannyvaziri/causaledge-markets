...
import { disconnectBroker, findBrokerOwnerByAccountHash, loadBrokerConnection, saveBrokerConnection } from './broker-store.js';
...
function accountSummaries(payload) {
  ...
  for (const item of list.slice(0, 20)) {
    const ref = String(item?.account_number || item?.accountNumber || item?.account_id || item?.accountId || '');
    const agenticAllowed = Boolean(item?.agentic_allowed ?? item?.agenticAllowed);
    if (ref && agenticAllowed) hashes.push(crypto.createHash('sha256').update(`robinhood:${ref}`).digest('hex'));
    const summary = {
    ...
    if (summary.maskedAccount || summary.agenticAllowed || summary.accountType) summaries.push(summary);
  }
  return { summaries, hashes: [...new Set(hashes)] };
}
...
export async function verifyRobinhoodConnection(ownerKey) {
  ...
  let accountResult = { summaries: [], hashes: [] };
  const accountTool = names.find((name) => ['get_accounts', 'list_accounts', 'accounts'].includes(name));
  if (accountTool) {
    try { accountResult = accountSummaries(parseToolJson(await callTool(token.accessToken, listed.sessionId, accountTool, {}))); } catch {}
  }

  for (const hash of accountResult.hashes) {
    const existingOwner = await findBrokerOwnerByAccountHash('robinhood', hash);
    if (existingOwner && existingOwner !== ownerKey) {
      await disconnectBroker(ownerKey, 'robinhood', 'Robinhood Agentic account ownership conflict blocked.');
      throw new Error('This Robinhood Agentic account is already linked to another CausalEdge user.');
    }
  }
  ...
  const connection = {
    ...
    agenticAccountHashes: accountResult.hashes,
    accountSummariesEncrypted: accountResult.summaries.length ? encryptBrokerValue(accountResult.summaries) : token.connection?.accountSummariesEncrypted || '',
  };
  await saveBrokerConnection(ownerKey, 'robinhood', connection, 'Robinhood Agentic MCP connection verified read-only.');
  return { connection, tools: names, accounts: accountResult.summaries };
}
